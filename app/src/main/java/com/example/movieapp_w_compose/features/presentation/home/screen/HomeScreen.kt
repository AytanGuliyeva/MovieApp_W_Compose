package com.example.movieapp_w_compose.features.presentation.home.screen
...
import com.example.movieapp_w_compose.features.presentation.home.viewModel.HomeViewModel
...
fun HomeScreenContent(
    ...
    LaunchedEffect(viewModel) {
        viewModel.singleEventFlow.collect { event ->
            when (event) {
                is HomeSingleEvent.OpenDetailMovieScreen -> {
                    ...
                    items(genreMovies) { movie ->
                        movie.posterPath?.let {
                            MovieItem(
                                title = movie.title,
                                posterPath = it,
                                onClick = { viewModel.handleAction(HomeUiAction.MovieClicked(movie.id)) }
                            )
                        }
                    }
                }
